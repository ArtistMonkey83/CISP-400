...
PolygonMatrix::PolygonMatrix(int _xPixels, int _yPixels): pointMatrix(2,0) //Fl::w() Fl::h()??
{
  xPixels = _xPixels;
  yPixels = _yPixels;
...
void PolygonMatrix::updatePixels()
{
  ...
  for(int i = 0; i < pointMatrix.getCols(); i++)//loop through each column of pointMatrix
  {
    ...
    Polygon::set_point(i,temp); //updating point in the graphics lib for later drawing ????Shape::set_point(temp)????
  }
