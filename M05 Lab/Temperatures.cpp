...
//os << "Year: " << y.year;
  ...
  {
    //os << setprecision(2) << fixed << setw(12) << y.year << '|';
    if (y.months[month_i].month != not_a_month)
    {
    //  os << setprecision(2) << fixed << setw(12) << y.year << '|'; //output the year
    //  os << setw(12) << int_to_month(month_i) << '|';   //output the month
      for (int day_i = 1; day_i < 31; day_i++)
      {
        //os <<  setw(12) << day_i << '|';
        //if (y.months[month_i].days[day_i].hours.at(day_i) == not_a_reading) break;//skipping it if its not a reading
        for (int hour_i = 0; hour_i < 24; hour_i++)
        {
          if (y.months[month_i].days[day_i].hours.at(hour_i) != not_a_reading)
          {
            ...
            os << setw(12) << day_i << '|';
            os << setw(12) << hour_i << '|';
            os << setw(12) << y.months[month_i].days[day_i].hours.at(hour_i) << '|' << std::endl;
          }
        }
      }
    }
