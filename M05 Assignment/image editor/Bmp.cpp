...
void Bmp::readHeader(istream& is)
{
    ...
    ///come to my office hour if you want to talk about the next line (difficult) binary operators used to get the multiple of four
...
void Bmp::readPixels(istream& is)
{
    ...
    for (int i = 0; i < height; i++)  //reading into a vector
    {
        int j;
        for (j = 0; j < width; j++)
        {
            b = (unsigned char)is.get(); //gets one byte at a time
...
void Bmp::write(string fileName) //called in main assumes
{
    //cout << "Generating header." << endl;
    createHeader(); //creates from array
    cout << "Writing to " << fileName << endl;
    ofstream os(fileName, ios::binary); //binary is used because it will keep track of the size of the file and will not terminate on the EOF value if used as a text file 
    writeHeader(os);
    writePixels(os);  //writing all the rgb values
